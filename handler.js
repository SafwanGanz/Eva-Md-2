...
module.exports = {
    async handler(chatUpdate) {
	    ...
	    global.sock = conn    
            ...
            global.Evn = '2023 COUTDOWN'
        
            global.thumbd = "https://telegra.ph/file/01615b2470978183a85ff.jpg"
            ...
            global.nd = pickRandom(['@Eva', 'Eva', 'Eva ', 'rasel', 'Eva', 'Eva', 'Eva', 'Eva', 'Eva', 'Eva', 'Eva', 'Eva', 'Eva', 'Eva', 'Eva', 'Eva', 'Eva', 'Eva', 'Eva'])
	    ...
	    let user = (m.isGroup ? participants.find(u => this.decodeJid(u.id) === m.sender) : {}) || {} // User Data
			...
			if (!['unbanchat.js', 'exec.js', 'exec2.js', 'creator.js', 'banned_list.js'].includes(name) && chat && chat?.isBanned && !isPrems) return // Kecuali ini, bisa digunakan
                    ...
                    if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                        this.sendBL(m.chat, await this.trans(lang, `Your limit is up, please buy via *${usedPrefix}buy(number)* or *${usedPrefix}buyall*\n\nBecome a premium user so that your limit is unlimited, if interested, please type *.uptoprem*`), wm, fla + "limit out", [[`Buy 1`, `.buy1`], [`Buy All`, `.buyall`], [`Up To Premium`, `.uptopremiu`]], m)
                        // this.reply(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}buy*`, m)
                        continue // Limit habis
                    }
                    if (plugin.level > _user.level) {
                        this.sendBL(m.chat, await this.trans(lang, `Level required *${plugin.level}* to use this command. Your level *${_user.level},* increase your Level by typing *${usedPrefix}levelup* or click the button below!`), wm, fla + "required level", [['Games', '.game'], [`Level Up`, `.levelup`]], m)
                ...
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            await this.sendBD(id, text, wm, pp, [[action === 'add' ? 'Welcome' : 'Goodbye', '@Eva']], {                      
                              ...
                              { 
                               jpegThumbnail: await (await fetch("https://telegra.ph/file/")).buffer(), 
...
To turn off this feature, type #enable delete
`
        this.sendB(msg.key.remoteJid, caption, wm, null, [[`Turn on Delete`, `.enable delete`]], msg, {mentions: [participant]})
...
global.dfail = async (type, m, conn) => {
    let msg = {
        ...
        moderator: `This Command is For *Moderators*`,
        premis: `This Command Only For *Premium Users*`,
        ...
        game: `The *GAME* feature is not active. Please contact @${global.owner[0]} to activate it`,
        ...
        restrict: `The *Admin* feature is not active Please contact @${global.owner[0]} to activate it`,
      }[type]
...
function clockString(ms) {
    ...
    return [h + ' hour ', m + ' Minute', s + ' scends'].map(v => v.toString().padStart(2, 0)).join(' ')
